<x-layouts.front title="AI Studio">
    ...
    <section class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-2">
        <div>
            ...
            <p class="font-body-md text-on-surface-variant">Turn any idea into structured tasks or a full Agile backlog — powered by Gemini AI.</p>
        </div>
        {{-- Mode Toggle --}}
        <div class="flex items-center bg-surface-container-high rounded-2xl p-1 gap-1 shadow-sm">
            <button id="btn-breakdown"
    ...
    {{-- Input Panel --}}
    <div class="bg-surface-container-lowest border border-outline/8 rounded-[28px] p-8 shadow-sm relative overflow-hidden">
        <div class="absolute top-0 right-0 w-48 h-48 bg-primary/5 rounded-full blur-2xl -translate-y-12 translate-x-12 pointer-events-none"></div>
        <div class="absolute bottom-0 left-0 w-32 h-32 bg-tertiary/5 rounded-full blur-xl translate-y-8 -translate-x-8 pointer-events-none"></div>
        ...
        <div class="relative z-10 space-y-6">
            <div>
                ...
                <div class="relative">
                    <textarea
                        ...
                        class="w-full bg-on-background/[0.03] border-0 border-b-2 border-on-surface-variant/20 py-4 px-1 font-body-md text-on-surface placeholder:text-on-surface-variant/40 focus:border-primary focus:ring-0 transition-all duration-300 resize-none rounded-t-lg"
                    ></textarea>
                    <span id="char-count" class="absolute bottom-3 right-3 text-[11px] text-on-surface-variant/50 font-label-sm">0 / 2000</span>
                </div>
            </div>
            ...
            {{-- Error Banner --}}
            <div id="error-banner" class="hidden bg-error/10 border border-error/30 text-error rounded-xl px-5 py-3 font-label-md flex items-center gap-3">
    ...
    <div id="results-area" class="hidden space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <button onclick="clearResults()" class="flex items-center gap-2 px-4 py-2 rounded-xl border border-outline/20 text-on-surface-variant font-label-md hover:bg-surface-container-high transition-all text-label-md">
        ...
        <div id="breakdown-results" class="hidden space-y-4">
            ...
            {{-- Save Tasks Bar --}}
            <div id="save-bar" class="hidden bg-primary-fixed/40 border border-primary/20 rounded-2xl px-6 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div class="space-y-3 flex-1">
                    <p class="font-label-md text-on-surface"><span id="selected-count">0</span> task(s) selected</p>
                    <div class="flex flex-wrap gap-3 items-center">
                        <select id="save-category" class="bg-surface border border-outline/20 rounded-lg font-label-md text-on-surface py-2 px-3 text-label-md focus:ring-primary focus:border-primary">
                            <option value="">— No Category —</option>
                            @foreach($categories as $cat)
                                <option value="{{ $cat->id }}">{{ $cat->name }}</option>
                            @endforeach
                        </select>
                        <input id="save-due-project-title" type="text" class="bg-surface border border-outline/20 rounded-lg font-label-md text-on-surface py-2 px-3 text-label-md focus:ring-primary focus:border-primary">
                   
                        <input id="save-due-date" type="date" class="bg-surface border border-outline/20 rounded-lg font-label-md text-on-surface py-2 px-3 text-label-md focus:ring-primary focus:border-primary">
                    </div>
                </div>
                <button
                    id="save-btn"
        ...
        <div id="backlog-results" class="hidden space-y-6">
            <div id="project-summary" class="bg-primary-container rounded-2xl p-6 text-on-primary-container relative overflow-hidden">
                ...
                <h3 id="project-title" class="font-display text-headline-md mb-2"></h3>
                <p id="project-desc" class="font-body-md opacity-90"></p>
            </div>
    ...
    @push('style')
    <style>
        .task-card { transition: all 0.25s cubic-bezier(0.34, 1.56, 0.64, 1); }
        .task-card:hover { transform: translateY(-3px); box-shadow: 0 12px 32px rgba(0,101,101,0.12); }
        .task-card.selected { border-color: #006565 !important; background: #f0fafa; box-shadow: 0 0 0 2px rgba(0,101,101,0.2); }
        .sprint-card { animation: slideUp 0.4s ease forwards; opacity: 0; }
        @keyframes slideUp {
            from { opacity: 0; transform: translateY(16px); }
            to   { opacity: 1; transform: translateY(0); }
        }
        #results-area { animation: fadeIn 0.35s ease; }
        @keyframes fadeIn { from { opacity: 0; } to { opacity: 1; } }
        .priority-high  { background: #ffe4e4; color: #a43c12; }
        .priority-med   { background: #fff3e0; color: #795200; }
    ...
    function switchMode(mode) {
        ...
        if (mode === 'breakdown') {
            btnBreakdown.className = `mode-btn flex items-center gap-2 px-5 py-2.5 rounded-xl font-label-md text-label-md transition-all duration-300 ${active}`;
            btnBacklog.className   = `mode-btn flex items-center gap-2 px-5 py-2.5 rounded-xl font-label-md text-label-md transition-all duration-300 ${inactive}`;
        } else {
            btnBacklog.className   = `mode-btn flex items-center gap-2 px-5 py-2.5 rounded-xl font-label-md text-label-md transition-all duration-300 ${active}`;
            btnBreakdown.className = `mode-btn flex items-center gap-2 px-5 py-2.5 rounded-xl font-label-md text-label-md transition-all duration-300 ${inactive}`;
        }
    ...
    async function generate() {
        ...
        try {
            ...
            if (!res.ok || data.error) {
                ...
                console.log('AI Generation Error:', data.error ?? 'Unknown error');
                
                return;
            }
            console.log(data);

            // Normalize AI-provided project title & summary from different response shapes
            currentProjectTitle = data.project_title || data.title || (data.backlog && data.backlog.project_title) || '';
            currentProjectSummary = data.summary || data.project_summary || (data.backlog && data.backlog.project_summary) || '';

            if (currentMode === 'breakdown') {
                renderTaskBreakdown(data.tasks, idea);
            } else {
                renderAgileBacklog(data.backlog, idea);
            }
    ...
    function renderTaskBreakdown(tasks, idea) {
        ...
        document.getElementById('results-subtitle').textContent = `From: "${idea.substring(0, 80)}${idea.length > 80 ? '...' : ''}"`;
        document.getElementById('results-summary').textContent = `From: "${currentProjectSummary.substring(0, 80)}${currentProjectSummary.length > 80 ? '...' : ''}"`;
        ...
        allGeneratedTasks.forEach((task, i) => {
            ...
            card.className = 'task-card bg-surface-container-lowest border border-outline/8 rounded-2xl p-5 cursor-pointer relative';
        ...
        allGeneratedTasks.forEach((_, i) => {
            const cb = document.getElementById(`task-check-${i}`);
            if (cb) { cb.checked = true; toggleTask(i, true); }
        });
    }
    ...
    function renderAgileBacklog(backlog, idea) {
        ...
        document.getElementById('results-title').textContent = backlog.project_title || 'Agile Backlog';
        document.getElementById('results-subtitle').textContent = `${backlog.sprints.length} sprint(s) generated`;

        document.getElementById('project-title').textContent = backlog.project_title || '';
        document.getElementById('project-desc').textContent  = backlog.project_summary || '';
        ...
        backlog.sprints.forEach((sprint, si) => {
            const colorClass = sprintColors[si % sprintColors.length];
            const delay = si * 80;

            const sprintEl = document.createElement('div');
            sprintEl.className = `sprint-card bg-surface-container-lowest border border-outline/8 rounded-2xl overflow-hidden border-l-4 ${colorClass}`;
            sprintEl.style.animationDelay = `${delay}ms`;

            const storiesHtml = (sprint.stories || []).map(story => `
                <div class="bg-surface rounded-xl p-5 border border-outline/6 space-y-3">
                    <div class="flex items-start justify-between gap-3">
                        ...
                        <div class="flex items-center gap-2 shrink-0">
                            <span class="px-2 py-1 rounded-full text-[11px] font-bold priority-${(story.priority||'low').toLowerCase()}">${(story.priority||'low').toUpperCase()}</span>
                            <span class="w-8 h-8 rounded-full bg-primary-fixed flex items-center justify-center text-on-primary-fixed-variant text-[12px] font-bold">${story.story_points ?? '?'}</span>
                        </div>
                    </div>
                    ...
                    ${story.acceptance_criteria && story.acceptance_criteria.length ? `
                    <div>
            ...
            const totalPoints = (sprint.stories || []).reduce((s, st) => s + (st.story_points || 0), 0);

            sprintEl.innerHTML = `
                <button onclick="toggleSprint(this)" class="w-full flex items-center justify-between p-6 text-left group">
                    <div>
                        <p class="font-label-sm text-label-sm uppercase tracking-wider text-on-surface-variant mb-1">Sprint ${sprint.sprint_number}</p>
                        <h4 class="font-display text-headline-md text-on-surface font-bold">${escHtml(sprint.name)}</h4>
                        <p class="font-body-md text-label-md text-on-surface-variant mt-1">${escHtml(sprint.goal)}</p>
                    </div>
                    <div class="flex items-center gap-4 shrink-0 ml-4">
                        <div class="text-right">
                            <p class="font-label-sm text-label-sm text-on-surface-variant">${(sprint.stories||[]).length} stories</p>
                            <p class="font-headline-md text-label-md font-bold text-primary">${totalPoints} pts</p>
                        </div>
                ...
                <div class="sprint-body px-6 pb-6 space-y-4">
                    ${storiesHtml}
                </div>
            `;
    ...
    function showSuccessToast(msg) {
        ...
        toast.className = 'fixed bottom-6 right-6 bg-primary text-on-primary px-6 py-4 rounded-2xl font-label-md shadow-xl flex items-center gap-3 z-50 animate-bounce';
        toast.innerHTML = `<span class="material-symbols-outlined">check_circle</span> ${escHtml(msg)} — <a href="{{ route('tasks.index') }}" class="underline font-bold">View Tasks →</a>`;
